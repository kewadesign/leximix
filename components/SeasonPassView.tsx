    ...
    return (
        <div className="h-full flex flex-col bg-[#0b1120] relative overflow-hidden animate-fade-in">
            ...
            <div className="p-4 flex items-center justify-between z-20 backdrop-blur-md bg-black/20 border-b border-white/5">
                <button onClick={onClose} className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 transition-colors border border-white/10">
            ...
            >
                <div className="flex items-center h-full px-[50vw] min-w-max gap-0 py-8">
                    ...
                    <div className="absolute top-1/2 left-0 right-0 h-2 bg-gray-800/50 -translate-y-1/2 z-0 backdrop-blur-sm"></div>
                    <div
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`absolute bottom-[55%] flex flex-col items-center justify-end h-32 w-full transition-all duration-300 ${isUnlocked ? 'opacity-100' : 'opacity-80 grayscale-[0.5]'}`}>
                                    <div className="absolute -top-8 text-xs font-black uppercase tracking-widest bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent drop-shadow-lg">Premium</div>
                                        ...
                                        <div className={`relative w-28 h-32 bg-gradient-to-b from-indigo-900 to-slate-900 rounded-2xl border-2 border-indigo-500/30 p-3 flex flex-col items-center justify-between shadow-xl hover:scale-110 transition-transform hover:z-20 group-hover:shadow-indigo-500/40 ${isClaimed ? 'ring-2 ring-yellow-400 bg-yellow-900/20 grayscale' : 'hover:shadow-indigo-500/20'}`}>
                                            ...
                                            <div className="flex-1 flex items-center justify-center relative overflow-hidden">
                                                ...
                                                {premiumReward.type === 'avatar' ? (
                                                    ...
                                                    <Zap size={48} className="text-yellow-400 drop-shadow-[0_0_10px_rgba(234,179,8,0.5)] animate-pulse z-10" />
                                                ) : (
                                                    <Gem size={24} className="text-blue-300 drop-shadow-lg z-10" />
                                                )}
                                            </div>

                                            <div className="text-center w-full relative z-10">
                                                <div className="text-[10px] font-bold text-indigo-200 truncate w-full">{premiumReward.type.toUpperCase()}</div>
                                                {premiumReward.amount && <div className="text-[9px] text-indigo-400 font-mono">x{premiumReward.amount}</div>}
                                            </div>
                                        ...
                                        <div className={`relative w-24 h-28 bg-gradient-to-b from-slate-800 to-slate-900 rounded-2xl border border-white/10 p-2 flex flex-col items-center justify-between shadow-lg hover:scale-105 transition-transform hover:z-20 group-hover:shadow-white/20 ${isClaimed ? 'opacity-50 grayscale' : 'hover:shadow-white/10'}`}>
                                            ...
                                            <div className="flex-1 flex items-center justify-center relative overflow-hidden">
                                                {freeReward.type === 'coins' ? (
                                                    <Coins size={32} className="text-yellow-400 drop-shadow-md z-10" />
                                                ) : freeReward.type === 'sticker' ? (
                                                    <Star size={32} className="text-pink-400 drop-shadow-md z-10" />
                                                ) : (
                                                    <Coins size={20} className="text-yellow-400 drop-shadow-md z-10" />
                                                )}
                                            </div>

                                            <div className="text-center w-full relative z-10">
                                                <div className="text-[9px] font-bold text-slate-300 truncate w-full">{freeReward.type.toUpperCase()}</div>
                                                {freeReward.amount && <div className="text-[8px] text-slate-500 font-mono">x{freeReward.amount}</div>}
                                            </div>
